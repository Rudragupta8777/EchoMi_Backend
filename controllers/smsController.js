...
const getLatestSmsByCall = async (req, res) => {
  try {
    ...
    if (forceFresh && userId) {
      console.log(
        `[SMS CONTROLLER] 🔄 Fresh data requested - triggering new SMS fetch for user: ${userId}`,
      );
    ...
    const smsMessages = await Sms.find(query)
      ...
      .select(
        "phoneNumber message sender timestamp smsType storageType callSid",
      );

    console.log(
      `[SMS CONTROLLER] 📊 Found ${smsMessages.length} messages, latest timestamp: ${smsMessages.length > 0 ? new Date(smsMessages[0].timestamp).toLocaleString() : "none"}`,
    );
...
const storeCallSms = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `📱 Storing ${smsMessages.length} SMS messages for call ${callSid}`,
    );
    ...
    // Store SMS messages
    const result = await SmsService.storeCallSms(
      userId,
      callSid,
      smsMessages,
      storageType,
