...
class SMSVerificationService {
  constructor() {
    ...
    // Organization aliases for better matching
    this.organizationAliases = {
      amazon: ["amazon", "amzn", "amazon delivery", "amz"],
      flipkart: ["flipkart", "fk", "flipkar", "ekart", "fk-"],
      swiggy: ["swiggy", "swigy", "swgy"],
      zomato: ["zomato", "zomto", "zmt"],
      dunzo: ["dunzo"],
  ...
  async checkForOTP(userId, company, callSid = null) {
    try {
      console.log(
        `[SMS SERVICE] Checking for ${company} OTP for user ${userId} - fetching latest messages`,
      );
      ...
      try {
        ...
        if (
          ...
          smsResponse.data.data.length > 0
        ) {
          console.log(
            `[SMS SERVICE] ✅ Fetched ${smsResponse.data.data.length} SMS messages for user`,
          );
          ...
          latest5.forEach((msg, idx) => {
            console.log(
              `[SMS SERVICE]   ${idx + 1}. ${new Date(msg.timestamp).toLocaleString()} - ${msg.sender}: "${msg.message.substring(0, 50)}..."`,
            );
          });

          const otpResult = this.extractOTPFromMessages(
            smsResponse.data.data,
            company,
          );
          ...
          if (otpResult.found) {
            console.log(
              `[SMS SERVICE] 🎯 Found real OTP for ${company}: ${otpResult.otp}`,
            );
            return {
              found: true,
              otp: otpResult.otp,
        ...
        console.error(
          `[SMS SERVICE]  Error fetching real SMS: ${smsError.message}`,
        );
        return {
          found: false,
          otp: null,
  ...
  extractOTPFromMessages(messages, company) {
    try {
      ...
      console.log(
        `[SMS SERVICE] Looking for ${companyLower} OTP in ${messages.length} messages`,
      );

      // Get all aliases for this organization
      const orgAliases = this.getOrganizationAliases(companyLower);
      console.log(
        `[SMS SERVICE] Using aliases for ${companyLower}:`,
        orgAliases,
      );
      ...
      console.log(
        `[SMS SERVICE] 📅 Sorted ${sortedMessages.length} messages by timestamp (newest first)`,
      );
      ...
      for (const message of sortedMessages) {
        ...
        console.log(
          `[SMS SERVICE] Checking message from ${message.sender}: "${message.message?.substring(0, 80)}${message.message?.length > 80 ? "..." : ""}"`,
        );
        console.log(
          `[SMS SERVICE]   📅 Timestamp: ${new Date(message.timestamp).toLocaleString()}`,
        );

        // Enhanced company matching with aliases - STRICT matching
        const matchesOrgAlias = orgAliases.some(
          (alias) => messageText.includes(alias) || sender.includes(alias),
        );

        // Additional contextual matching for delivery/order messages
        ...
        if (isRelevantMessage) {
          console.log(
            `[SMS SERVICE]  ✅ Relevant ${company} message found from ${message.sender}`,
          );
          ...
          for (const pattern of otpPatterns) {
            ...
            if (match && match[1]) {
              ...
              if (
                potentialOTP.length >= 4 &&
                potentialOTP.length <= 8 &&
                ...
                const priority = this.calculateOTPPriority(
                  pattern,
                  messageText,
                  potentialOTP,
                  message.timestamp,
                ...
                console.log(
                  `[SMS SERVICE]  🔍 Found potential OTP "${potentialOTP}" (priority: ${priority}) from ${new Date(message.timestamp).toLocaleString()}`,
                );
          ...
          if (hasDigits) {
            console.log(
              `[SMS SERVICE]  ⚠️  Message has digits but not flagged as relevant: "${message.message?.substring(0, 100)}"`,
            );
          }
        }
      }
      ...
      if (foundOTPs.length > 0) {
        ...
        console.log(
          `[SMS SERVICE] 🎯 SELECTED BEST OTP: "${bestOTP.otp}" from ${new Date(bestOTP.timestamp).toLocaleString()} (priority: ${bestOTP.priority})`,
        );
        console.log(
          `[SMS SERVICE] 🏆 Total OTPs found: ${foundOTPs.length}, selected highest priority`,
        );
      ...
      console.log(
        `[SMS SERVICE] ❌ No OTP found in ${messages.length} messages for ${company}`,
      );
      return { found: false, otp: null };
    } catch (error) {
  ...
  extractTrackingFromMessages(messages, company) {
    try {
      ...
      console.log(
        `[SMS SERVICE] Looking for ${companyLower} tracking IDs in ${messages.length} messages`,
      );
      ...
      for (const message of messages) {
        ...
        const isRelevantMessage =
          ...
          messageText.includes("tracking") ||
          messageText.includes("order");

        if (isRelevantMessage) {
          console.log(
            `[SMS SERVICE]  Checking for tracking ID in message from ${message.sender}: "${messageText}"`,
          );
          ...
          for (const pattern of trackingPatterns) {
            ...
            if (matches) {
              console.log(
                `[SMS SERVICE]    Pattern matched: ${pattern} → ${matches[1]}`,
              );
              if (pattern.global) {
                ...
                for (const match of allMatches) {
                  if (match[1] && match[1].length >= 6) {
                    ...
                    console.log(
                      `[SMS SERVICE]    Added tracking ID: ${match[1].toUpperCase()}`,
                    );
                  }
                }
              } else {
                ...
                if (matches[1] && matches[1].length >= 6) {
                  ...
                  console.log(
                    `[SMS SERVICE]    Added tracking ID: ${matches[1].toUpperCase()}`,
                  );
                }
              }
            }
      ...
      const uniqueTrackingIds = [...new Set(trackingIds)].filter(
        (id) =>
          id.length >= 6 && id.length <= 20 && !/^\d{4}$|^\d{6}$/.test(id), // Exclude OTPs
      );

      console.log(
        `[SMS SERVICE]  Found tracking IDs: ${uniqueTrackingIds.join(", ")}`,
      );
  ...
  async verifyTrackingId(providedTrackingId, company, callSid) {
    try {
      console.log(
        `[TRACKING VERIFICATION] Verifying tracking ID: "${providedTrackingId}" for ${company}`,
      );

      if (!callSid) {
        console.error(
          "[TRACKING VERIFICATION] ❌ No callSid provided for verification",
        );
        return {
          verified: false,
          message:
            "I don't have the delivery information. Let me send a notification for approval.",
        };
      }
      ...
      if (
        ...
        const trackingResult = this.extractTrackingFromMessages(
          smsResponse.data.data,
          company,
        );
        ...
        if (trackingResult.found) {
          console.log(
            `[TRACKING VERIFICATION] Found tracking IDs in SMS: ${trackingResult.trackingIds.join(", ")}`,
          );
          ...
          const providedNormalized = providedTrackingId
            .replace(/[^A-Z0-9]/gi, "")
            .toUpperCase();
          console.log(
            `[TRACKING VERIFICATION] Normalized provided ID: "${providedNormalized}"`,
          );
          console.log(
            `[TRACKING VERIFICATION] Comparing provided: "${providedTrackingId}" (normalized: "${providedNormalized}") with extracted: [${trackingResult.trackingIds.join(", ")}]`,
          );

          // CRITICAL FIX: Don't allow empty or very short tracking IDs
          if (!providedNormalized || providedNormalized.length < 4) {
            console.log(
              `[TRACKING VERIFICATION] ❌ Provided tracking ID is empty or too short: "${providedNormalized}"`,
            );
            return {
              verified: false,
              message:
                "I couldn't understand the tracking ID. Let me send a notification for manual approval.",
            };
          }

          const isMatch = trackingResult.trackingIds.some((id) => {
            const extractedNormalized = id
              .replace(/[^A-Z0-9]/gi, "")
              .toUpperCase();
            console.log(
              `[TRACKING VERIFICATION] Comparing "${providedNormalized}" with "${extractedNormalized}"`,
            );
            return (
              extractedNormalized === providedNormalized ||
          ...
          if (isMatch) {
            console.log(
              `[TRACKING VERIFICATION] ✅ Tracking ID verified successfully`,
            );
            return {
              verified: true,
            ...
            console.log(
              `[TRACKING VERIFICATION] ❌ Tracking ID does not match. Expected: ${trackingResult.trackingIds.join(" or ")}`,
            );
            return {
              verified: false,
              message:
                "The tracking ID you provided doesn't match our records. Please check and try again.",
            };
          }
        } else {
          console.log(
            `[TRACKING VERIFICATION] ❌ No tracking IDs found in SMS messages`,
          );
          return {
            verified: false,
            message:
              "I couldn't find any tracking information in the messages. Let me send a notification for manual approval.",
          };
        }
      } else {
        console.log(
          `[TRACKING VERIFICATION] ❌ No SMS messages found for verification`,
        );
        return {
          verified: false,
          message:
      ...
      console.error(
        "[TRACKING VERIFICATION] Error verifying tracking ID:",
        error,
      );
      return {
        verified: false,
        message:
          "I'm having trouble verifying the tracking ID. Let me send a notification for manual approval.",
      };
    }
  }
  ...
  async requestUserApproval(userId, fcmToken, company, callerNumber, callSid) {
    try {
      ...
      console.log(
        `[APPROVAL REQUEST] 🔍 Creating approval request: ${approvalId} for ${company}`,
      );
      console.log(
        `[APPROVAL REQUEST] 📱 FCM Token available: ${fcmToken ? "Yes" : "No"}`,
      );
      console.log(
        `[APPROVAL REQUEST] 📞 Caller: ${callerNumber}, CallSid: ${callSid}`,
      );
      ...
      console.log(
        `[APPROVAL REQUEST] 🔔 Sending FCM notification for ${company} OTP approval...`,
      );
      ...
      console.log(
        `[APPROVAL REQUEST] ✅ FCM notification sent successfully:`,
        fcmResult,
      );
      return {
        sent: true,
      ...
      console.error(
        `[APPROVAL REQUEST] ❌ Failed to send FCM approval notification:`,
        error,
      );
   ...
   * @param {string} organization - Organization name (lowercase)
   * @returns {string[]} Array of aliases
   */
  getOrganizationAliases(organization) {
    const normalized = organization.toLowerCase().trim();

    // Return aliases if we have them
    if (this.organizationAliases[normalized]) {
      return this.organizationAliases[normalized];
    }

    // Otherwise return the organization itself
    return [normalized];
  }
}
